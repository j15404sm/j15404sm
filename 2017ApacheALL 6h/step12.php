...
function read_log($log_fname, $dbh) {
	...
	while ($line = fgets($fp)) {
		...
		$remote_hostname	= $dbh->quote($m[1]);	#%h	Remote hostname 
		$remote_logname		= $dbh->quote($m[2]);	#%l	Remote logname
		$remote_user		= $dbh->quote($m[3]);	#%u	Remote user
		$request_time		= $dbh->quote($m[4]);	#%t	Time the request was received
		$first_line			= $dbh->quote($m[5]);	#%r	First line of request
		$status				= $dbh->quote($m[6]);	#%s	Status
		$bytes				= $dbh->quote($m[7]);	#%b	Size of response in bytes
		$referer			= $dbh->quote($m[8]);	#%{Referer}i
		$user_agent			= $dbh->quote($m[9]);	#%{User-Agent}i
...
read_log("access_log/access_log", $dbh);
